...
两个小实验已完成，完整结果见 [事件 bracket 与共享平台开放实验报告](oty2_rsa2_g1_r1_event_bracket_and_shared_platform_experiments_20260718.md)。

- 实验 A 共 6 条线程：GM17 三车和 GM11 PV001 的事件 bracket 包含 preferred GT 最小半径；GM19 PV002 推翻“完全显露等于最近点”，修订 bracket 后仅得到稀疏支持；GM19 PV004 因窗口结束只能保留单侧接近趋势。
- 实验 B 共 7 个多车窗口：optical 左右顺序与 SAR theta 顺序为 `7/7` 一致；表观尺度—相对半径排序在 5 个窗口局部一致、2 个窗口受姿态/截断反例推翻。PV007/PV008 仍不能靠方向区分，但可由成对方位顺序反驳交换。

以下保留原始实验责任，作为结果的可追踪设计依据。
...
10. **两个小实验结果？** 事件 bracket 比旧分位更符合真实图像，但必须允许仍截断时达到最近点和窗口 censoring；多车方位顺序在 7 个窗口全部稳定，表观尺度—相对半径关系则被 2 个姿态/截断窗口反驳。
...
- [长轴方向语义纠偏](../../docs/OTY2_RSA2_G1_R1_ROTATED_BOX_LONG_AXIS_SEMANTIC_CORRECTION.md)及其审计表；
- [事件 bracket 与共享平台开放实验报告](oty2_rsa2_g1_r1_event_bracket_and_shared_platform_experiments_20260718.md)及两份小型案例表；
- A01–A12 共 12 张视觉证据图，目录为 `docs/reviews/assets/20260718_rsa2_g1_r1_mechanism_reinterpretation/`。
