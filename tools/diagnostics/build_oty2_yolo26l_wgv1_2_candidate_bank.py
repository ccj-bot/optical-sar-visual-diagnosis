...
DEFAULT_RAW_TABLES = [
    ...
    "yolo26l_local_detection_table.csv"
]
...
def build_bank(
    ...
    for raw_table in raw_tables:
        ...
        for raw_row in read_csv(raw_table):
    ...
    for selected_row in selected_manifest:
        key = (
            selected_row["scene_id"],
            int(selected_row["optical_frame_num"]),
...
def coverage_rows(bank_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    ...
    for scene, start, end, window_id in CRITICAL_WINDOWS:
        ...
        if len(frames_with_raw) == len(expected_frames):
            status = "all_candidate_available"
        elif frames_with_raw:
            status = "partial_all_candidate_available"
        ...
        rows.append(
            {
                ...
                "frames_with_raw_all_candidate_table": str(len(frames_with_raw)),
...
def main() -> int:
    ...
    bank = build_bank(selected_manifest, target_families, raw_tables)
    coverage = coverage_rows(bank)
    tf_contrast = target_family_contrast_rows(target_families, bank)
